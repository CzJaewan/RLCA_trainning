...
from model.utils import test_circle_init_pose, test_circle_goal_point
...
class StageWorld():
    ...
    def laser_scan_callback(self, scan):
        ...
        cut_scan = full_scan[208:1232]
        
        scan_re = []

        for i in range(512):
            num = i * 2
            min_scan = min(cut_scan[num], cut_scan[num+1])
            scan_re.append(min_scan)

        self.scan = np.array(scan_re)
        self.laser_cb_num += 1

    def laser_dead_callback(self, dead_scan):
        self.scan_dead = np.array(dead_scan.ranges)
        self.scan_dead[np.isnan(self.scan_dead)] = 0.9
        self.scan_dead[np.isinf(self.scan_dead)] = 0.9
        dead_scan_min = np.min(self.scan_dead)
        if dead_scan_min < 0.85:
            self.is_dead = True
    ...
    def generate_random_pose(self):
        x = round(np.random.uniform(-12, 12), 3)
        y = round(np.random.uniform(-12, 12), 3)
    ...
    def generate_random_goal(self):
        ...
        while (dis_origin > 14 or dis_goal > 14 or dis_goal < 7) and not rospy.is_shutdown():
            x = round(np.random.uniform(-12, 12),3)
            y = round(np.random.uniform(-12, 12),3)
            dis_origin = np.sqrt(x ** 2 + y ** 2)
            dis_goal = np.sqrt((x - self.init_pose[0]) ** 2 + (y - self.init_pose[1]) ** 2)
    ...
    def reset_pose_test(self):
        ...
        reset_pose = test_circle_init_pose(self.index)
        self.init_pose = reset_pose
        self.control_pose(reset_pose)
        self.is_dead = False


    def generate_goal_point_test(self):
        self.goal_point = test_circle_goal_point(self.index)
        self.pre_distance = 0
        self.distance = copy.deepcopy(self.pre_distance)