...
class StageWorld():
    ...
    def laser_dead_callback(self, dead_scan):
        ...
        self.scan_dead[np.isnan(self.scan_dead)] = 1.1
        self.scan_dead[np.isinf(self.scan_dead)] = 1.1
        dead_scan_min = np.min(self.scan_dead)
        if dead_scan_min < 1.08:
            self.is_dead = True
    ...
    def generate_goal_point(self):
        ...
        self.first_distance = self.pre_distance 
    ...
    def get_reward_and_terminate(self, t):
        ...
        reward_g = (self.pre_distance - self.distance) * 10/self.first_distance * 1.0
        reward_c = 0
        reward_w = 0
        reward_t = 0
        result = 0
    ...
    def generate_random_goal(self):
        ...
        while (dis_origin > 14 or dis_goal > 10 or dis_goal < 7) and not rospy.is_shutdown():
            x = round(np.random.uniform(-12, 12),3)
            y = round(np.random.uniform(-12, 12),3)
            dis_origin = np.sqrt(x ** 2 + y ** 2)
            dis_goal = np.sqrt((x - self.init_pose[0]) ** 2 + (y - self.init_pose[1]) ** 2)
