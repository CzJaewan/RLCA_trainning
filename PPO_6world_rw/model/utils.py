...
def get_init_pose_24(index):
    init_pose_list = [[-7.00, 11.50, np.pi], [-18.00, 11.50, 0.00], [-12.50, 17.00, np.pi*3/2],     #3
                      [-2.00, 16.00, -np.pi/2], [3.00, 16.00, -np.pi/2],                            #2
                      [10.00, 4.00, np.pi/2], [14.00, 4.00, np.pi/2], [18.00, 4.00, np.pi/2],       #3
                      [-2.5, -2.5, 0.00], [3.5, -2.5, np.pi],                                       #2
                      [-2.5, -18.5, np.pi/2], [1.5, -18.5, np.pi/2], [5.5, -18.5, np.pi/2],         #3
                      
                      [-6.00, -10.00, np.pi], [-10.15, -4.29, np.pi*7/5],           
                      [-16.85, -6.47, np.pi*9/5], [-16.85, -13.53, np.pi*11/5],     
                      [-10.15, -15.71, np.pi*13/5],                                                 #5
                      
                      [10.00, -17.00, np.pi/2], [14.00, -17.00, np.pi/2], [18.00, -17.00, np.pi/2], 
                      [10.00, -2.00, -np.pi/2], [14.00, -2.00, -np.pi/2], [18.00, -2.00, -np.pi/2]] #6
    return init_pose_list[index]


def get_goal_point_24(index):
    goal_list = [[-18.0, 11.5], [-7.0, 11.5], [-12.5, 4.0],                                         #3
                 [-2.0, 3.0], [3.0, 3.0],                                                           #2
                 [10.0, 10.0], [14.0, 10.0], [18.0, 10.0],                                          #3
                 [3.5, -2.5], [-2.5, -2.5],                                                         #2
                 [-2.5, -5.5], [1.5, -5.5], [5.5, -5.5],                                            #3
                 [-18.0, -10.0], [-13.85, -15.71], [-7.15, -13.53], [-7.15, -6.47], [-13.85, -4.29],#5
                 ]
    return goal_list[index]
