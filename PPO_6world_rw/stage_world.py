...
class StageWorld():
    ...
    def generate_goal_point(self):
        if self.index > 17 and self.index < 24:
            self.goal_point = self.generate_random_goal()
        else:
            self.goal_point = get_goal_point_24(self.index)
    ...
    def get_reward_and_terminate(self, t):
        ...
        reward_g = (self.pre_distance - self.distance) * 2.6
        reward_c = 0
        reward_w = 0
        reward_t = 0
        reward_ct = 0
    ...
    def reset_pose(self):
        if self.index > 17 and self.index < 24:
            ...
            reset_pose = get_init_pose_24(self.index)
