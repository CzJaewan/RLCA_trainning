...
from geometry_msgs.msg import Twist, Pose, Point32, PoseWithCovarianceStamped
...
class StageWorld():
    def __init__(self, beam_num, index, num_env, robot_radius):
        ...
        self.object_state_sub = rospy.Subscriber(amcl_pose_topic, PoseWithCovarianceStamped, self.amcl_pose_callback)
    ...
    def amcl_pose_callback(self, amcl_pose):
        Quaternious = amcl_pose.pose.orientation
        Euler = tf.transformations.euler_from_quaternion([Quaternious.x, Quaternious.y, Quaternious.z, Quaternious.w])
        self.state_GT = [amcl_pose.pose.position.x, amcl_pose.pose.position.y, Euler[2]]
