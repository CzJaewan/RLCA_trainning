...
def run(comm, env, policy, policy_path, action_bound, optimizer):
	...
	full_obs = env.get_laser_observation()
	full_obs_stack = deque([full_obs, full_obs, full_obs, full_obs, full_obs, full_obs, full_obs, full_obs, full_obs, full_obs, full_obs])

        # get_state
        # obs = env.get_laser_observation()
        obs_stack = deque([full_obs_stack[0], full_obs_stack[5], full_obs_stack[10]])
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            #-------------------------------------------------------------------------
            # get informtion
            r, terminal, result = env.get_reward_and_terminate(step, real_action)
            ep_reward += r
            global_step += 1
            ...
            left = full_obs_stack.popleft()
            #left???????
            full_obs_stack.append(s_next)
            obs_stack = deque([full_obs_stack[0], full_obs_stack[5], full_obs_stack[10]])
...
if __name__ == '__main__':
    ...
    output_file = './log/' + hostname + 'radius_test' + '/output.log'
    cal_file = './log/' + hostname + 'radius_test' + '/cal.log'
    ...
    if rank == 0:
        ...
        file = policy_path + '/epi_73000.pth'
