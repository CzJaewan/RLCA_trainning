...
class StageWorld():
    ...
    def get_reward_and_terminate(self, t, action):
        terminate = False
        ...
        reward_g = (self.pre_distance - self.distance) * 3spq
        reward_c = 0
        reward_w = 0
        reward_t = 0
        reward_ct = 0
        ...
        if np.abs(w_dot) > 0.2:
            reward_wdot = -0.25 * np.abs(w_dot)

        reward = reward_g + reward_c + reward_w + reward_ct + reward_t + reward_wdot
